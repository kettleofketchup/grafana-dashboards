...
> **Resolution status updated 2026-06-01 after implementation (Phases A/B/D shipped; Phase C pending workstation `sudo`).**

- **Sidecar consumes v2beta1.** ✅ **RESOLVED — switched to grafana-operator, not the sidecar.** The design originally planned to strip the envelope to `.spec` for the ConfigMap sidecar. During implementation we instead deployed **grafana-operator v5.5.2** (`oci://ghcr.io/grafana-operator/helm-charts`) and ship each dashboard as a `GrafanaDashboard` CR carrying the full v2beta1 envelope in `spec.json`. Gotchas hit + fixed: (1) the CRD kind is `GrafanaDashboard` (single word), not `Dashboard`; (2) Helm parses the embedded `{{cpu}}`/`{{device}}` legend templates as Go-template actions — `render_to_chart.py` escapes `{{` as `{{ "{{" }}`; (3) the operator authenticates to the external Grafana via the `grafana-admin` Secret, but Grafana's SQLite admin password had drifted from the Secret — reset via `grafana cli admin reset-admin-password` (a self-healing PostSync Job was designed for this; implement if drift recurs).
- **Loki service name and port.** ✅ **RESOLVED.** Loki is SingleBinary in the `monitoring` namespace; Service is `loki:3100`. IngressRoute + datasource URL both use `loki.monitoring.svc:3100`.
- **SealedSecret vs SOPS.** ✅ **RESOLVED.** Cluster uses neither — raw `Secret` + `ServerSideApply=true` + Helm `lookup` for idempotency (matches the existing `grafana-admin-secret.yaml` pattern). **Plus a Traefik gotcha:** basicAuth requires a Secret with EXACTLY one `users` key; the workstation cred Secret was split into `workstation-ingest-htpasswd` (Traefik) + `workstation-ingest-credentials` (workstation fetches via kubectl).
- **Wildcard TLS cross-namespace.** ✅ **RESOLVED (new, discovered in impl).** The `home-kettle-sh-tls` wildcard cert lived only in `traefik` ns; ingest IngressRoutes in `monitoring` 404'd until Reflector annotations were added to the Certificate (`reflector.v1.k8s.emberstack.com/reflection-*`) replicating it into monitoring/loki/authentik.
- **Foundation-SDK pin.** ✅ Confirmed working at `git+a8c311b58` (v2beta1 builders). Verified the exact API surface against the installed wheel (`Builder[DataQueryKind]` inheritance, `SortOrder.DESCENDING`, `ThresholdsConfig` builder location, no `Panel.time_from`).
- **GPU exporter packaging.** ⏳ **PENDING (Phase C).** `grafana-alloy` confirmed in Arch `extra` repo; `nvidia-gpu-exporter-bin` is the AUR package. Fallback (upstream binary under systemd) still documented in the plan.
- **AUR package staleness.** ⏳ Phase C — same as above.
- **cAdvisor as a 2026 standalone agent.** ⏳ **PENDING (Phase C).** `just alloy::configure` does a cgroupv2 preflight; if `prometheus.exporter.cadvisor` is missing from the installed Alloy, `alloy run --dry-run` fails clearly → switch to [`cgroup_exporter`](https://github.com/treydock/cgroup_exporter). Not yet exercised (needs workstation install).
- **`host_id` privacy.** ✅ Confirmed: ships `sha256(machine_id)[:16]`.
...
- **NVIDIA exporter metric names.** ⏳ **PENDING (Phase C, C6).** Panel PromQL assumes `nvidia_smi_*` names (e.g. `nvidia_smi_clocks_current_graphics_clock_hz`); these vary by exporter version. C6 preflights `curl 127.0.0.1:9835/metrics` and updates `panels/timeseries.py` if they differ before relying on them.
